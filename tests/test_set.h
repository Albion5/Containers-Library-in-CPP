...
TEST(set, Insert) {
  s21::set<std::string> s21_set = {"This", "is", "s21", "set"};
  std::set<std::string> std_set = {"This", "is", "s21", "set"};
  auto s21_pr = s21_set.insert("!");
  auto std_pr = std_set.insert("!");
...
TEST(set, InsertMany) {
  s21::set<typename std::vector<int>> s21_set;
  std::vector<int> vec1 = {1, 2, 3, 4, 5};
  std::vector<int> vec2 = {4, 8, 13, 0, 55};
  s21::set<typename std::vector<int>> std_set = {vec1, vec2};
  s21_set.insert_many(vec1, vec2);
  auto s21_it = s21_set.begin();
  auto std_it = std_set.begin();
  for (; s21_it != s21_set.end(); ++s21_it, ++std_it) {
    EXPECT_TRUE(*std_it == *s21_it);
...
TEST(set, InsertMany2) {
  std::set<char> std_set = {'a', 'l', 'i', 'n', 'a'};
  s21::set<char> s21_set;
  s21_set.insert_many('a', 'l', 'i', 'n', 'a');
  auto s21_it = s21_set.begin();
  auto std_it = std_set.begin();
  for (; s21_it != s21_set.end(); ++s21_it, ++std_it) {
    EXPECT_TRUE(*std_it == *s21_it);
